...
首个原创版本。项目独立实现 MZI 网格分解、传播、非理想性模型与教学动画，
并附一份单独收录的对照样例用于回归验证。完整技术对照见 [docs/review.md](docs/review.md)。

### 修复

- **退化分支取值写反，导致结构化矩阵静默算错**（技术对照 B1）。
  `mzi_transfer_matrix(0, 0)` 是交换阵而非单位阵，对照样例的两个退化分支恰好互换。
