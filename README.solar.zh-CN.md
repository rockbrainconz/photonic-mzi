...
> **分支状态：实验性。** 本文是 `experiment/solar-incoherent` 的入口说明。
> 该分支研究非相干日光强度计算，不是给主线相干 MZI 处理器更换光源。
...
| 日照波动 | 不适用 | 同时参考通道只能消除公共模波动 |
| 光谱假设 | 当前主线未建模 | 当前为波长积分等效误差，尚非逐波长模型 |
| 主要类 | `PhotonicMatrixProcessor` | `IncoherentSolarProcessor` |

两套处理器共享“验证矩阵乘加”的上层目标，但没有共享传播方程或器件拓扑。
...
若同时参考探测器测得 `Pref=C(t)`，并补回输入与权重的已知编码标度，即可恢复
`Mx+b`。这个归一化只能消除所有通道共享的标量变化，不能消除局部阴影、光谱失配、
通道不均匀、正负探测臂失配或独立探测噪声。
