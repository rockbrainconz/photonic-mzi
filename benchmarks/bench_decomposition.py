"""
复杂度对比：参考实现（每台 MZI 构造 N×N 全矩阵再相乘）vs 本实现（就地两行更新）。
...
from tests._reference_original import (  # noqa: E402
    PhotonicMatrixProcessor as ReferenceOPU,
)
...
def main() -> None:
    ...
    for N in [8, 16, 32, 64, 128]:
        ...
        t_ref_c = timeit(lambda A=A: ReferenceOPU(A), rep)
        t_new_c = timeit(lambda A=A: PhotonicMatrixProcessor(A), rep)

        ref, new = ReferenceOPU(A), PhotonicMatrixProcessor(A)
